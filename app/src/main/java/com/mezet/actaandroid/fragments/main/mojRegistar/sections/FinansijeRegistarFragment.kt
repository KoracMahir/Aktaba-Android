...
import com.mezet.actaandroid.adapters.FInansijeAdapter
...
class FinansijeRegistarFragment : Fragment(), CompanyByIdView {
    ...
    override fun onCreateView(
        ...
        val v= inflater.inflate(R.layout.fragment_finansije_registar, container, false)
        ...
        token?.let { FinansijePresenter(this).getClientCompanyFin(companyId, it) }
    ...
    override fun getCompany(companyPayload: CompanyPayload?) {
        val adapter =context?.let { companyPayload?.let { it1 -> FInansijeAdapter(it1, it) } }
        val recyclerView = view?.findViewById(R.id.finansije_recycler) as RecyclerView
