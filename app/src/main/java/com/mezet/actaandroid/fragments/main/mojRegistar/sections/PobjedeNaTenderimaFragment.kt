...
class PobjedeNaTenderimaFragment : Fragment(),PobjedeTenderiView {
    ...
    override fun onCreateView(
        ...
        val page = 1
        token?.let { PobjedeTenderiPresenter(this).getTenderiPobjede(companyId,page,"", it) }
        return v
    }
    fun back(){
    ...
    override fun getWinners(winners: TenderWinners?) {
        val adapter =context?.let { winners?.let { it1 -> PobjedeAdapter(it1, it) } }
        val recyclerView = view?.findViewById(R.id.recycler_pobjede) as RecyclerView
